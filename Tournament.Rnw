...
\section{Expected value and variance of the total score}
\subsection{Expected value of the total score}
...
The average total score of the $n$ questions is the sum of all the average question scores:
\begin{equation}
E(score_{total})=E(\sum_{i=1}^{i=n}score_{i})=\sum_{i=1}^{i=n}p_{i}(1-p_{i}) + \sum_{i=1}^{i=n}(q_{i}-p_{i})^{2}
\label{eq:meantotal}
\end{equation}

\subsection{Variance of the total score}

The variance of the score on question $i$ is the average value of the square of the difference between the score on question $i$  and the expected score on question $i$ (which is given by equation \ref{eq:expscorei}):
\begin{equation}
\label{eq:varscorei}
  \begin{split}
    var(score_{i})&=E\left( (score_{i} - E(score_{i}))^2  \right) \\
    &= \left[(1-q_{i})^{2}-p_{i}(1-p_{i}) - (q_{i}-p_{i})^{2}  \right]^{2}p_{i} +  \left[q_{i}^{2}-p_{i}(1-p_{i}) - (q_{i}p_{i})^{2}  \right]^{2}(1-p_{i})\\
    &= \left[1 - 2q_{i} + q_{i}^{2} -p_{i}+ p_{i}^{2} -q_{i}^{2} + 2q_{i}p_{i} - p_{i}^{2}  \right]^{2}p_{i} + \left[ q_{i}^{2} - p_{i} + p_{i}^{2} - q_{i}^{2} +2q_{i}p_{i}-p_{i}^{2} \right]^2(1-p_{i}) \\
    &= \left[(1-2q_{i}) - p_{i}(1-2q_{i}) \right]^{2}p_{i} + \left[-p_{i}(1-2q_{i})  \right]^{2}(1-p_{i}) \\
    &= (1-2q_{i})^{2}p_{i}- p_{i}^{2}(1-2q_{i})^{2} \\
    &= p_{i}(1-p_{i})(1-2q_{i})^{2}
  \end{split}
\end{equation}

The variance of the total score of all $n$ questions is the sum of all the variances of the question scores:
\begin{equation}
var(score_{total})=\sum_{i=1}^{i=n}var(score_{i}))=\sum_{i=1}^{i=n}p_{i}(1-p_{i})(1-2q_{i})^{2}
\label{eq:vartotal}
\end{equation}

\section{Expected value and variance of total score in multiple tournaments}
Because the total score of a tournament with $n$ questions

\begin{itemize}
  \item is the sum of $n$ scores (one for each question), 
  \item all questions are considered to be independent 
  \item all question scores have the same weight
\end{itemize}

we can reasonably conclude that the Central Limit Theorem is applicable which states that under these conditions the total score of $n$ questions will be approximately normally distributed with parameters $\mu=E(score_{total}$ and $\sigma=\sqrt{var(score_{total}}$.

However, when we have multiple ($N$) tournaments the values of $q_{i}$ can be different in each tournament. This depends on the model that we will use to simulate the decisions of the forecaster. We therefore adapt our formulas, adding the index $j \quad (j=1 \ldots N)$, where $j$ indicates the tournament number.

The average of the total score of tournament $j$ is:
\begin{equation}
E_{j}(score_{total})=\sum_{i=1}^{i=n}p_{i}(1-p_{i}) + \sum_{i=1}^{i=n}(q_{i,j}-p_{i})^{2}
\label{eq:meantournament}
\end{equation}

The variance of the total score of tournament $j$ is::
\begin{equation}
var_{j}(score_{total})=\sum_{i=1}^{i=n}p_{i}(1-p_{i})(1-2q_{i,j})^{2}
\label{eq:vartournament}
\end{equation}
...
This forecaster is faultless: for every question she ''knows" the exact probability. So for every question her choice $q_{i,j}$ of the probability of occurence ot the event is exactly equal to $p_{i}$. 

We can calculate the expected value and the variance of the total score of the Perfect Forecaster using Equation~\ref{eq:meantournament} and Equation~\ref{eq:vartournament}. We get:
\begin{equation}
\label{eq:perfect}
  \begin{split}
    E(S_{perfect})&=\sum_{i=1}^{i=n}p_{i}(1-p_{i}) \\
    var(S_{perfect})&=\sum_{i=1}^{i=n}p_{i}(1-p_{i})(1-2p_{i})^{2}
  \end{split}
\end{equation}

As we can see $q_{i,j}$ is not present any more. As the values of $p_{i}$ are known, all tournaments will have the same average value and the same variance.
...
mu.perfect <- sum(tour_run$p*(1-tour_run$p))
var.perfect <- sum(tour_run$p*(1-tour_run$p)*(1-2*tour_run$p)^2)
std.perfect <- sqrt(var.perfect)
norm.perfect <- data.frame(x=seq(10,30,by=0.1),y =0)
norm.perfect$y <- dnorm(norm.perfect$x,mu.perfect,std.perfect)
ggplot() +
  geom_histogram(data=total_score_perfect, aes(x = result, y = ..density..), 
                 binwidth=0.5, fill="lightpink1",  size=0.2) +
  geom_density(data=total_score_perfect, aes(x = result, y = ..density..)) +
  geom_line(data=norm.perfect, aes(x=x,y=y), color="red", size=1) +
  geom_vline(xintercept=mu.perfect, color="red", size=0.8) +
  xlim(0, 50) +
...
The average score of the \emph{Perfect Forecaster} is fixed once we have chosen the values of $p_{i}$ and the number of questions $n$. In this example the average perfect score is \Sexpr{round(mu.perfect,2)}. The variance of the total scores of the \emph{Perfect Forecaster} is \Sexpr{round(var.perfect)}. From the variance we find the standard deviation (it's the square) and it is equal to \Sexpr{round(std.perfect,2)}. Theory predict that the total scores will be normally distibuted with parameters $\mu=$\Sexpr{round(mu.perfect,2)} and $\sigma=$\Sexpr{round(std.perfect,2)}. In Figure~\ref{fig:Perfect} the red curve shows the normal distribution with these parameters. It fits the simulation quite well.
...
Her average total score is given by Equation~\ref{eq:meantotal} and the variance by Equation~\ref{eq:vartotal}:
\begin{equation}
\label{eq:ignorant}
  \begin{split}
E_{j}(S_{ignorant})&=\sum_{i=1}^{i=n}p_{i}(1-p_{i}) + \sum_{i=1}^{i=n}(q_{i,j}-p_{i})^{2} = E(S_{perfect}) + \sum_{i=1}^{i=n}(q_{i}-p_{i})^{2} \\
var_{j}(S_{ignorant})&=\sum_{i=1}^{i=n}p_{i}(1-p_{i})(1-2q_{i,j})^{2}
  \end{split}
\end{equation}

Because the second term in the first equation is always positive, the average total score of the Ignorant Forecaster will always be higher that the average score of the Perfect Forecaster. Both equations depend on the values of $q_{i,j}$, which means that when we rerun the simulations in another tournament, we will get other values of $q_{i,j}$ and therefore other values for the expected value of the total score and for the variance of the total score.

<<label=Ignorant,fig=TRUE,include=FALSE, echo=FALSE>>=
set.seed(2018) # setting the random number generator
# set up data frame with known probabilities p and randomly chosen probabilities q in range [0,1]
total_score_ignorant <- data.frame(result=rep(0,N), mu=0, var=0)
# defining the real probabilities for each question: 10 questions with prob=0.05; 10 with prob=0.15 and so on till prob=0.95
tour_run <- data.frame(p = rep(prob.event, n/10), q = 0, event=0)
# simulating N tournaments
for (i in (1:N)) {
  ...
  total_score_ignorant$mu[i] <- sum(tour_run$p*(1-tour_run$p)) + sum((tour_run$q-tour_run$p)^2)
  total_score_ignorant$var[i] <- sum(tour_run$p*(1-tour_run$p)*(1-2*tour_run$q)^2)
}
gem.result.ignorant <- mean(total_score_ignorant$result)
mu.ignorant <- mean(total_score_ignorant$mu)
# std.ignorant <- sqrt(mean(total_score_ignorant$var)) The theoretical value
std.ignorant <- sd(total_score_ignorant$result) # The real std: much larger
norm.ignorant <- data.frame(x=seq(20,50,by=0.1), y =0)
norm.ignorant$y <- dnorm(norm.ignorant$x,mu.ignorant,std.ignorant)
ggplot() +
  ...
  geom_line(data=norm.ignorant, aes(x=x,y=y), color="blue", size=1) +
  geom_vline(xintercept=mu.perfect, color="red", size=0.5) +
  geom_vline(xintercept=gem.result.ignorant, color="blue", size=0.8) +
  annotate("text", x= mu.perfect, y= 0.04, label="Perfect", color="red") +
  annotate("text", x= mu.perfect, y= 0.03, label="Forecaster", color="red") +
...
ggplot(data=total_score_better, aes(x = result, y = ..density..)) +
  ...
  geom_vline(xintercept=mu.perfect, color="red", size=0.5, linetype=2) +
  annotate("text", x= mu.perfect, y= 0.21, label="Perfect", color="red") +
  annotate("text", x= mu.perfect, y= 0.20, label="Forecaster", color="red") +
