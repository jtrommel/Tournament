...
\title{Notes on the "Prediction Tournament Paradox" (prof. David J. Aldous)}
...
\chapter{Another forecaster model}

\section{Distribution of the total scores for different contestants}
If we take a more gentle model for the \emph{Better Forecaster}, and allow her to pick a random number in the range $[p_{i}-\sigma,p_{i}+\sigma]$ the total scores of the realistic forecaster get better (Figure~\ref{fig:FiveTRJ}) compared with Figure~\ref{fig:FiveContestants}.
