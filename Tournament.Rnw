...
<<echo=FALSE>>=
sigma.ignorant <- sqrt((gem.result.ignorant - perfect_score)/n)
@
The value for $\sigma$ for the \emph{Ignorant Forecaster} is $\Sexpr{round(sigma.ignorant,2)}$. The range of choice for the \emph{Better Forecaster} will thus be $[p_{i}-\Sexpr{round(sigma.ignorant,2)},p_{i}+\Sexpr{round(sigma.ignorant,2)}]$. \sidenote{When the lower limit is smaller than 0, it will be trimmed to 0. Conversely: an upper limit above 1 will be trimmed to 1}
...
# set up data frame with known probabilities p and 
# randomly chosen probabilities q in range [p-sigma,p+sigma]
N <- 500 # number of tournaments
n <- 100 # number of questions per run
percentage_win_C1 <- data.frame(matrix(0, ncol = 7, nrow = 7))
colnames(percentage_win_C1) <- as.character(seq(0, 0.3, by=0.05))
rownames(percentage_win_C1) <- as.character(seq(0, 0.3, by=0.05))
for (p1 in (1:7)) {
  for (p2 in (p1:7)) {
...
tour <- data.frame(tournament = rep(seq(1:N), each=n*nC),
                   ...
                   p = rep(seq(0.05, 0.95, by=0.1), N*nC*n/10))
...
tour$event <- rbinom(n*nC*N,1,tour$p)
tour$question.score <- (tour$event - tour$q)^2
tour %>% 
  group_by(tournament, contestant, sigma) %>% 
  summarise(score=sum(question.score)) -> tournament.table
tournament.table <- tournament.table[order(tournament.table$tournament, tournament.table$score),]
tournament.table$win <- rep(c(1,rep(0,nC-1)),N)
tournament.table %>% group_by(contestant, sigma) %>% summarise(som=sum(win)) -> endscore
endscore$percentage <- endscore$som/N
endscore
@
