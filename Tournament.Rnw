...
The variance of the total score is the sum of all the variances of the question scores:
\begin{equation}
...
This forecaster is faultless: for every question she ''knows" the exact probability. So for every question her choice $q_{i}$ of the probability of occurence ot the event is exactly equal to $p_{i}$. 

We can calculate the expected value and the variance of the total score of the Perfect Forecaster using Equation~\ref{eq:meantotal}. The second term in the equation for the expected value is 0 because the Perfect Forecaster always chooses $q_{i}=p_{i}$:
\begin{equation}
\label{eq:perfect}
  \begin{split}
...
\label{eq:ignorant}
  \begin{split}
E(S_{ignorant})&=\sum_{i=1}^{i=n}p_{i}(1-p_{i}) + \sum_{i=1}^{i=n}(q_{i}-p_{i})^{2} = E(S_{perfect}) + \sum_{i=1}^{i=n}(q_{i}-p_{i})^{2} \\
var(S_{ignorant})&=\sum_{i=1}^{i=n}p_{i}(1-p_{i})(1-2q_{i})^{2}
  \end{split}
\end{equation}

Because the second term is always positive in the first equation, the average total score of the Ignorant Forecaster will always be higher that the average score of the Perfect Forecaster\sidenote{Both equations depend on the values of $q_{i}$, which means that when we rerun the simulations, we will get other values of $q_{i}$ and therefore other values for the expected value of the total score and for the variance of the total score}.
