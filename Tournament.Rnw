...
One run = 100 questions with true probabilities 0.05, 0.15, ..., 0.95, each appearing 10 times. The forecaster chooses for each question a random probability\sidenote{random uniform distribution within [0,1]} in the range [0,1]. The score will of course be worse (a higher value) than when the forecaster each time makes a perfect choice. Because of the random nature of the choice, the forecaster will be lucky in some sets of 100 questions, and sometimes she/he will be unlucky. The end result will be spread around a mean value. The spread is given by the error $\epsilon$.
...
ggplot(data=total_score, aes(x = result, y = ..density..)) +
  ...
  annotate("text", x= perfect_score, y= 0.03, label="score", color="red") +
  annotate("text", x= gem.result, y= 0.04, label="random", color="blue") +
  annotate("text", x= gem.result, y= 0.03, label="choice", color="blue") +
  geom_vline(xintercept=gem.result, color="blue", size=0.8) +
  xlim(0, 50) +
  labs(title = "Distribution of Aldous scores for 10.000 tournaments", 
       subtitle = "Forecaster makes random choice",
       x = "Aldous Score", 
       y = "density") +
  JT.theme
@
...
A better forecaster is someone who does not make a random choice of the probability, but will give a value which is closer to the real probability. Instead of making a random choice in the range [0,1], he/she will give a value in the range $[p-\sigma,p+\sigma]$ where $\sigma$ is the RMS error. Values for the forecasted probabilities must of course not go below 0 or above 1.
...
ggplot(data=total_score, aes(x = result, y = ..density..)) +
  ...
  annotate("text", x= perfect_score, y= 0.04, label="perfect", color="red") +
  annotate("text", x= perfect_score, y= 0.03, label="score", color="red") +
  annotate("text", x= gem.result, y= 0.04, label="better", color="blue") +
  annotate("text", x= gem.result, y= 0.03, label="choice", color="blue") +
  geom_vline(xintercept=gem.result, color="blue", size=0.8) +
  xlim(0, 50) +
  labs(title = "Distribution of Aldous scores for 10.000 tournaments", 
       subtitle = "Better Forecaster [p-sigma, p+sigma]",
       x = "Aldous Score", 
       y = "density") +
  JT.theme
@
